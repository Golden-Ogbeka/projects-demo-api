...
import { recordVendorManagementWebOperation } from "../database/index.js";
...
const resolveField = (field: string, variables: Record<string, unknown>): unknown => {
  if (scalarFields.has(field)) {
    if (field === "getCountryUrl") return "http://localhost:5050/vendor-management-web";
...
export const GraphqlController = () => {
  const Handle = async (
    ...
    recordVendorManagementWebOperation(operationName, rootFields, variables);
